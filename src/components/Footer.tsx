    ...
    try {
      const res = await fetch('https://api.web3forms.com/submit', {
        ...
        body: JSON.stringify({
          ...
          subject: '📩 New Newsletter Subscription — CreativeDesk Solutions',
          from_name: 'Newsletter Signup',
          email: nlEmail,
          message: `New subscriber: ${nlEmail}`,
          botcheck: '',
        }),
      });
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-[1.6fr_1fr_1fr_1.1fr_1.5fr] gap-8 sm:gap-10 pb-10 sm:pb-14 border-b border-white/10">
          ...
          <div className="flex flex-col gap-4">
            ...
            {/* Newsletter form */}
            <form onSubmit={handleNewsletterSubmit} noValidate className="flex flex-col xs:flex-row gap-2">
              <input
                ref={inputRef}
                type="email"
                value={nlEmail}
                ...
                className={`flex-1 min-w-0 px-4 py-2.5 rounded-full bg-white/5 border text-sm text-white placeholder-slate-500 outline-none transition-colors
                  ${
                    nlStatus === 'error'
              ...
              <button
                type="submit"
                disabled={nlStatus === 'loading' || nlStatus === 'success'}
                className="btn-primary btn-sm flex-shrink-0 whitespace-nowrap disabled:opacity-60 disabled:cursor-not-allowed transition-opacity"
              >
                {nlStatus === 'loading' ? (
                ...
                ) : nlStatus === 'success' ? '✓ Joined' : 'Join'}
              </button>
            </form>
            {/* Feedback message */}
            {nlMessage && (
              <p className={`text-xs font-medium transition-all ${
                nlStatus === 'success' ? 'text-green-400' : 'text-red-400'
              }`}>
                {nlMessage}
              </p>
            )}
