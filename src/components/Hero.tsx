...
export default function Hero() {
  return (
    ...
    >
      ...
      <div
        className="absolute top-[18%] left-[20%] w-8 h-8 rounded-full border-2 border-white/20 border-dashed animate-spin-slow pointer-events-none hidden lg:block"
        style={{ zIndex: 3 }}
      />
      <div
        className="absolute top-[75%] left-[12%] text-2xl font-light text-white/30 animate-float pointer-events-none hidden lg:block select-none"
        style={{ zIndex: 3 }}
      >
        +
      </div>
      <div
        className="absolute top-[25%] right-[15%] w-6 h-6 border-2 border-white/20 rotate-12 animate-float-delayed pointer-events-none hidden lg:block"
        style={{ zIndex: 3 }}
      />
      <div
        className="absolute bottom-[22%] left-[45%] w-3 h-3 rounded-full bg-white/20 animate-float pointer-events-none hidden lg:block"
        style={{ zIndex: 3 }}
      />
      ...
      {/* ── Main content ── */}
      <div
        className="relative max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 pt-28 sm:pt-36 pb-24 sm:pb-20
      ...
      >
        <div className="flex flex-col items-center text-center z-10 w-full max-w-3xl">
          {/* Welcome Badge */}
          <div
            className="flex items-center justify-center gap-2 px-3 sm:px-3.5 py-1.5 rounded-full mb-5 sm:mb-6
          ...
          <h1
            className="font-jakarta font-extrabold text-slate-900 
          ...
          <p className="text-slate-600 text-sm sm:text-base md:text-lg leading-relaxed mb-7 sm:mb-8 max-w-xl text-center mx-auto px-2 sm:px-0">
            We build blazing-fast platforms, mobile applications, and automated
            workflows that streamline your operations and scale your growth.
          </p>
          ...
          <div className="flex flex-col xs:flex-row items-center justify-center flex-wrap gap-3 sm:gap-4 mb-8 sm:mb-10 w-full px-4 sm:px-0">
            <a
              href="/contact"
              className="btn-primary shadow-[0_4px_24px_rgba(95,41,199,0.35)] flex items-center gap-2 px-5 sm:px-6 py-3 sm:py-3.5 text-sm w-full xs:w-auto justify-center"
            >
          ...
          <div className="flex items-center justify-center gap-3 pt-5 sm:pt-6 border-t border-slate-200/60 w-full max-w-xs sm:max-w-md mx-auto">
            <div
              className="flex -space-x-2.5 animate-pulse-subtle"
              aria-hidden="true"
            >
              <img
                src="https://images.unsplash.com/photo-1534528741775-53994a69daeb?auto=format&fit=crop&w=150&h=150&q=80"
                alt="User 1"
                className="w-8 h-8 sm:w-9 sm:h-9 rounded-full border-2 border-white shadow-sm object-cover flex-shrink-0"
              />
              <img
            ...
            <p className="text-xs sm:text-sm font-jakarta font-semibold text-slate-600">
              Trusted by <span className="text-brand-600 font-bold">1.3L+</span>{" "}
              Audience
            </p>
          </div>
        </div>
